...
-(void)CreateInitView
{
    ...
    [title setFont:[UIFont fontWithName:@"Arial-BoldMT" size:20]];
    ...
    [cancle addTarget:self action:@selector(outcancleLogin:) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.nameField.placeholder = @"外部用户名";
    ...
    [loginButton addTarget:self action:@selector(outloginInServer:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [rButton.titleLabel setFont:[UIFont fontWithName:@"DFWaWa-W7-HKP-BF" size:11.0]];
    ...
    NSArray *array = [NSArray arrayWithObjects:@"外部用户名",@"手机",@"请输入真实姓名",@"请输入工作单位",@"请输入手机号码", nil];
    
    for (int i = 0; i<2; i++) {
        UIView *review = [[UIView alloc] initWithFrame:CGRectMake(60, 50+35*i, 200, 30)];
    ...
    reButton.frame = CGRectMake(60, 125 , 200, 35);
...
-(void)outloginInServer:(id)sender
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(outloginInServer:)]) {
        [self.delegate outloginInServer:sender];
    }
}
...
-(void)registerInServer:(id)sender
{
    ...
    NSString *phone = [(UITextField *)[self viewWithTag:kLoginTextTag+1] text];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(outregisterInService:WithMobile:)]) {
        [self.delegate outregisterInService:name WithMobile:phone];
    }
}
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(outcancleLogin:)]) {
        [self.delegate outcancleLogin:sender];
    }
}
