...
-(void)selectDate:(int)date {
    ...
    NSDateComponents *comps = [gregorian components:NSYearCalendarUnit | NSMonthCalendarUnit |  NSDayCalendarUnit | NSTimeZoneCalendarUnit fromDate:self.currentMonth];
    [comps setDay:date];
...
//                           NSDayCalendarUnit) fromDate: localeDate];
    self.currentMonth = localeDate; //clean month
...
{       
    ...
    CGRect rectArrowLeft = CGRectMake(83, 0, 16, 40);
    CGRect rectArrowRight = CGRectMake(221, 0, 16, 40);
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSString *date = [formatter stringFromDate:self.currentMonth];
    if([date hasSuffix:@"January"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"January" withString:@"01"];
    else if ([date hasSuffix:@"February"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"February" withString:@"02"];
    else if ([date hasSuffix:@"March"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"March" withString:@"03"];
    else if ([date hasSuffix:@"April"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"April" withString:@"04"];
    else if ([date hasSuffix:@"May"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"May" withString:@"05"];
    else if ([date hasSuffix:@"June"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"June" withString:@"06"];
    else if ([date hasSuffix:@"July"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"July" withString:@"07"];
    else if ([date hasSuffix:@"August"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"August" withString:@"08"];
    else if ([date hasSuffix:@"September"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"September" withString:@"09"];
    else if ([date hasSuffix:@"October"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"October" withString:@"10"];
    else if ([date hasSuffix:@"November"])
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"November" withString:@"11"];
    else 
        labelCurrentMonth.text = [date stringByReplacingOccurrencesOfString :@"December" withString:@"12"];
...
//    [weekdays moveObjectFromIndex:0 toIndex:6];
    NSMutableArray *weekdays = [[NSMutableArray alloc] initWithObjects:@"周一",@"周二",@"周三",@"周四",@"周五",@"周六",@"周日", nil];
...
//    CGContextDrawImage(context, rectangleGrid,[UIImage imageWithName:@"BG_DATE"].CGImage);
    CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0xeed86f"].CGColor);
    CGContextFillPath(context);
    ...
    //Grid dark lines
    CGContextSetStrokeColorWithColor(context, [UIColor colorWithHexString:@"0xeed86f"].CGColor);
    CGContextBeginPath(context);
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        CGContextDrawImage(context, CGRectMake(targetX+1, targetY, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight), [UIImage imageNamed:@"date_grid.png"].CGImage);
        // BOOL isCurrentMonth = NO;
        if (i<firstWeekDay) { //previous month
            targetDate = (prevMonthNumDays-firstWeekDay)+(i+1);
            NSString *hex = (isSelectedDatePreviousMonth) ? @"0xb3720c" : @"b3720c";
            ...
            NSString *hex = (isSelectedDateNextMonth) ? @"0xb3720c" : @"b3720c";
            CGContextSetFillColorWithColor(context, 
                                           [UIColor colorWithHexString:hex].CGColor);
        } else { //current month
            // isCurrentMonth = YES;
            targetDate = (i-firstWeekDay)+1;
            NSString *hex = (isSelectedDatePreviousMonth || isSelectedDateNextMonth) ? @"0xb3720c" : @"0xb3720c";
            CGContextSetFillColorWithColor(context, 
                                           [UIColor colorWithHexString:hex].CGColor);
        }
...
//            CGContextAddRect(context, rectangleGrid);
            CGContextDrawImage(context, rectangleGrid, [UIImage imageNamed:@"selected_date.png"].CGImage);
            CGContextFillPath(context);
...
//            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0x383838"].CGColor);
        ...
        [date drawInRect:CGRectMake(targetX+2, targetY+10, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:@"HelveticaNeue" size:17] lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
...
-(id)init {
    ...
    if (self) {
        ...
        isAnimating=NO;
        UIImageView *dateHead = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, kVRGCalendarViewWidth, 35)];
        dateHead.image = [UIImage imageNamed:@"date_grid.png"];
        [self addSubview:dateHead];
        [dateHead release];
        
        dateHead = [[UIImageView alloc] initWithFrame:CGRectMake(85, 12, 12, 15)];
        dateHead.image = [UIImage imageNamed:@"leftarrow.png"];
        [self addSubview:dateHead];
        [dateHead release];
        
        dateHead = [[UIImageView alloc] initWithFrame:CGRectMake(223, 12, 12, 15)];
        dateHead.image = [UIImage imageNamed:@"rightarrow.png"];
        [self addSubview:dateHead];
        [dateHead release];
        ...
        labelCurrentMonth.textColor = [UIColor colorWithHexString:@"0xb3720c"];
        labelCurrentMonth.textAlignment = kTextAlignmentCenter;
